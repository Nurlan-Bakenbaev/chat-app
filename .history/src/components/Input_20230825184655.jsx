...
const Input = () => {
  ...
  const handleSend = async () => {
    ...
    if (img) {
      ...
      uploadTask.on(
        ...
        async () => {
          ...
          await setDoc(doc(db, "chats", data.chatId), messagesArrayUnion, {
            merge: true,
          });
        }
      );
    ...
    await setDoc(doc(db, "userChats", currentUser.uid), userChatsUpdates, {
      merge: true,
    });

    // Update or create the chat document in the "userChats" collection for the other user
    await setDoc(doc(db, "userChats", data.user.uid), userChatsUpdates, {
      merge: true,
    });
